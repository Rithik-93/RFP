...
app.post("/api/send-rfp", async (req, res) => {
  try {
    ...
    const vendors = await import("@rfp/database").then((m) =>
      m.prisma.vendor.findMany({
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const toolDescriptions = functionDeclarations
      .map(
        (fn) =>
...
3. NEVER make up data or hallucinate - always use tools to get real information
...
- When you call createRFPs, the tool returns: {"successful": 1, "rfps": [{"id": "actual-uuid", "title": "...", ...}]}
- EXTRACT the actual "id" field from the "rfps" array in the tool result
- USE that exact ID in subsequent calls like generateRFPEmailPreview
- NEVER use placeholder IDs like "rfp_12345" or "RFP_ID_FROM_PREVIOUS_CREATION"
- When you call createVendors, extract the actual email addresses from the "vendors" array in the result
- If user refers to "this RFP" or "the RFP", look back in conversation history for the most recent createRFPs tool result and extract the ID
- If unclear which RFP, list active RFPs and ask user to clarify

RFP CREATION WORKFLOW:
1. After calling createRFPs tool:
   - EXTRACT the RFP ID from the tool result: result.rfps[0].id
...
2. When user confirms, call generateRFPEmailPreview:
   - Use the ACTUAL RFP ID you extracted from step 1
   - If user mentions vendor names, call listVendors first to get their emails
   - Pass the actual vendor emails from listVendors result
3. After getting email preview, show it to user. User can:
...
EXAMPLE FLOW:
User: "Create RFP for laptops"
You: {"type": "tool_call", "tool": "createRFPs", "args": {...}}
[Tool returns: {"successful": 1, "rfps": [{"id": "cm4xabc123", "title": "Laptop Procurement", ...}]}]
You: {"type": "response", "message": "Created RFP 'Laptop Procurement' (ID: cm4xabc123). Would you like to send this to vendors?"}
User: "Yes, send to Rithik"
You: {"type": "tool_call", "tool": "listVendors", "args": {}}
[Tool returns: [{"name": "Rithik", "email": "rithik7899@gmail.com", ...}]]
You: {"type": "tool_call", "tool": "generateRFPEmailPreview", "args": {"rfpId": "cm4xabc123", "vendorEmails": ["rithik7899@gmail.com"]}}
...
CRITICAL: Your response will be parsed with JSON.parse(). Do NOT wrap it in markdown. Just return raw JSON. And very strictly you cannot send both tool_call and response in the same message.
...
REMINDER: Return ONLY JSON. No markdown, no code blocks.`;
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      const historyText = workingConversation
        .map(
          (msg: any) =>
...
app.post("/api/process-email-reply", async (req, res) => {
  try {
    ...
    if (!rfpVendor) {
      ...
      return res
        .status(404)
        .json({ error: "No matching RFP found for this email" });
    }
    ...
    for (const attachment of attachments) {
      try {
        ...
        if (mimeType.startsWith("image/") || mimeType === "application/pdf") {
          ...
          console.log(
            `Added attachment to Gemini: ${attachment.filename} (${mimeType})`
          );
        }
      } catch (error) {
        console.error(
    ...
    if (aiResult.type === "PROPOSAL" && aiResult.data) {
      ...
      console.log(
        `Created Proposal from ${from} (${attachments.length} attachments)`
      );
    } else {
      console.log(
        `Email is ${aiResult.type}, no Proposal created (only RFPReply)`
      );
    }
